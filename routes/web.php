...
Route::prefix('student')->name('student.')->middleware(['auth', 'check.maintenance'])->group(function () {
    ...
    // Student Notification Check Route
    Route::get('/notifications/check', function() {
        $user = Auth::user();

        // Kunin ang mga announcements
        ...
        // I-save ang current time sa session
...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'check.maintenance'])->group(function () {
    ...
    Route::put('/departments/{id}/assign-years', [App\Http\Controllers\Admin\DepartmentManagementController::class, 'assignYearsToDepartment'])->name('department.assign-years');
    
    // Archive/Restore (TEMPORARY - remove if not needed)
    ...
    // Student Management (BAGONG CONTROLLER)
    ...
    // Staff Management (BAGONG CONTROLLER)
    ...
    // Department Management (BAGONG CONTROLLER)
    ...
    // Officer Management (BAGONG CONTROLLER)
    ...
    // Clearance Management (BAGONG CONTROLLER)
    ...
    // Admin Profile (BAGONG CONTROLLER)
    ...
    // Activity Logs (BAGONG CONTROLLER)
...
Route::prefix('staff')->name('staff.')->middleware(['auth', 'check.maintenance'])->group(function () {
    ...
    Route::post('/logout', [DepartmentStaffController::class, 'logout'])->name('staff.logout');  // <-- DAPAT staff.logout

    // Year Requirements Routes
...
Route::get('/officer/export-verified-csv', [OfficerController::class, 'exportVerifiedCSV'])->name('officer.export.csv');
...
Route::get('/student/clearance/view-slip-pdf', [ClearanceRequestController::class, 'viewSlipPdf'])->name('student.clearance.view-pdf');
Route::get('/student/clearance/view-slip', [ClearanceRequestController::class, 'viewSlip'])->name('student.clearance.view');
