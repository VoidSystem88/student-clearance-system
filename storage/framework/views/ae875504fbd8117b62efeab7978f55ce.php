<?php $__env->startSection('title', 'Maintenance Control'); ?>
<?php $__env->startSection('header', 'System Maintenance Control'); ?>
...
<div class="bg-white rounded-xl shadow-sm p-6">
    <div class="flex justify-between items-center mb-5 border-b pb-3">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('support.maintenance.logout')); ?>" class="text-sm text-gray-500 hover:text-red-600 transition">
                <i class="fas fa-sign-out-alt mr-1"></i> Lock Access
            </a>
            <?php if($isMaintenanceMode || $isReadOnlyMode): ?>
    ...
    <!-- Status Banner -->
    <?php if($isMaintenanceMode || $isReadOnlyMode): ?>
    <div class="mb-6 p-4 rounded-lg <?php echo e($isMaintenanceMode ? 'bg-red-50 border border-red-200' : 'bg-yellow-50 border border-yellow-200'); ?>">
        <div class="flex items-center gap-3">
            <i class="fas <?php echo e($isMaintenanceMode ? 'fa-skull-crosswalk text-red-600' : 'fa-eye-slash text-yellow-600'); ?> text-2xl"></i>
            <div class="flex-1">
                <h4 class="font-bold <?php echo e($isMaintenanceMode ? 'text-red-800' : 'text-yellow-800'); ?>">
                    <?php echo e($isMaintenanceMode ? '🔴 FULL SHUTDOWN ACTIVE' : '🟡 SOFT SHUTDOWN ACTIVE'); ?>

                </h4>
                <p class="text-sm"><?php echo e($maintenanceMessage); ?></p>
                <?php if($maintenanceEndTime): ?>
                    <p class="text-xs mt-1">Expected end: <?php echo e(\Carbon\Carbon::parse($maintenanceEndTime)->format('F d, Y g:i A')); ?></p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <div class="mt-8">
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto">
            <?php $__empty_1 = true; $__currentLoopData = $recentMaintenance; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="border-l-4 <?php echo e($log->mode === 'full' ? 'border-red-400 bg-red-50' : 'border-yellow-400 bg-yellow-50'); ?> p-3 rounded-r-lg shadow-sm">
                <div class="flex justify-between items-start">
                    <span class="font-semibold <?php echo e($log->mode === 'full' ? 'text-red-700' : 'text-yellow-700'); ?>">
                        <?php echo e($log->mode === 'full' ? 'FULL SHUTDOWN' : 'SOFT SHUTDOWN'); ?>

                        <?php if($log->is_active): ?><span class="ml-2 text-xs bg-red-500 text-white px-2 py-0.5 rounded-full">ACTIVE</span><?php endif; ?>
                    </span>
                    <span class="text-xs text-gray-400"><?php echo e($log->created_at->format('M d, Y g:i A')); ?></span>
                </div>
                <p class="text-sm mt-1"><?php echo e(Str::limit($log->message, 100)); ?></p>
                <p class="text-xs text-gray-500 mt-1">Initiated by: <?php echo e($log->initiatedBy->name ?? 'Unknown'); ?>

                    <?php if($log->end_time): ?> | Expected end: <?php echo e(\Carbon\Carbon::parse($log->end_time)->format('M d, g:i A')); ?> <?php endif; ?>
                </p>
            </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-5">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <div class="mt-4">
                        <a href="<?php echo e(route('admin.download.backup')); ?>" 
...
document.getElementById('softShutdownForm')?.addEventListener('submit', async function(e) {
    ...
    Swal.fire({ title: 'Enable Soft Shutdown?', text: `System will become read-only for ${duration} hour(s).`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Yes, enable' }).then(async (result) => {
        if (result.isConfirmed) {
            ...
            const response = await fetch('<?php echo e(route("support.maintenance.soft")); ?>', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token }, body: JSON.stringify({ message, duration_hours: parseInt(duration) }) });
    ...
    fullShutdownForm.addEventListener('submit', async function(e) {
        ...
        Swal.fire({ title: '⚠️ ENABLE FULL SHUTDOWN ⚠️', text: `System offline for ${duration} hour(s).`, icon: 'error', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Yes, shutdown' }).then(async (result) => {
            if (result.isConfirmed) {
                ...
                const response = await fetch('<?php echo e(route("support.maintenance.full")); ?>', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token }, body: JSON.stringify({ message, duration_hours: parseInt(duration) }) });
                const data = await response.json();
    ...
    disableBtn.addEventListener('click', async function() {
        Swal.fire({ title: 'Disable Maintenance Mode?', text: 'System will return to normal.', icon: 'question', showCancelButton: true, confirmButtonColor: '#3085d6', confirmButtonText: 'Yes' }).then(async (result) => {
            if (result.isConfirmed) {
                ...
                const response = await fetch('<?php echo e(route("support.maintenance.disable")); ?>', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token } });
                const data = await response.json();
                if (data.success) Swal.fire('Success', data.message, 'success').then(() => location.reload());
                else Swal.fire('Error', data.error || 'Failed', 'error');
            }
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.support', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/vol15_6/infinityfree.com/if0_42013478/voidclearancesystem.gt.tc/htdocs/resources/views/support/maintenance.blade.php ENDPATH**/ ?>