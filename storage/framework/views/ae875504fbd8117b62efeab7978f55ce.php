...
<div class="bg-white rounded-xl shadow-sm p-6">
    <div class="flex justify-between items-center mb-5 border-b pb-3">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('support.maintenance.logout')); ?>" 
                ...
                <div class="border-l-4 <?php echo e($log->mode === 'full' ? 'border-red-400 bg-red-50' : 'border-yellow-400 bg-yellow-50'); ?> p-3 rounded-r-lg shadow-sm">
                    <div class="flex justify-between items-start">
                        <span class="font-semibold <?php echo e($log->mode === 'full' ? 'text-red-700' : 'text-yellow-700'); ?>">
                            <i class="fas <?php echo e($log->mode === 'full' ? 'fa-exclamation-triangle' : 'fa-eye-slash'); ?> mr-1"></i>
                    ...
                    <div class="text-xs text-gray-500 mt-1">
                        Initiated by: <?php echo e($log->initiatedBy->name ?? 'Unknown'); ?>

                        <?php if($log->end_time): ?>
                            | Expected end: <?php echo e(\Carbon\Carbon::parse($log->end_time)->format('M d, g:i A')); ?>
...
document.getElementById('softShutdownForm')?.addEventListener('submit', async function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const response = await fetch('<?php echo e(route("support.maintenance.soft")); ?>', {
                method: 'POST',
    ...
    fullShutdownForm.addEventListener('submit', async function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const response = await fetch('<?php echo e(route("support.maintenance.full")); ?>', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token },
                    body: JSON.stringify({ message, duration_hours: parseInt(duration) })
                });
    ...
    disableBtn.addEventListener('click', async function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const response = await fetch('<?php echo e(route("support.maintenance.disable")); ?>', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token }
                });
