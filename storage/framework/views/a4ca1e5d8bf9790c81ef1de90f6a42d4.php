        ...
        <div class="flex-1 flex items-center justify-center px-4 py-8 md:py-12">
            <div class="text-center max-w-4xl mx-auto">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <!-- STUDENT LOGIN -->
                    <button onclick="openLoginPanel()" class="btn-primary px-8 py-3 rounded-xl font-semibold text-white flex items-center justify-center gap-2 min-w-[180px]">
                        <i class="fas fa-user-graduate"></i> Student Login
                    </button>

                    <!-- STAFF LOGIN (Admin, Support, Staff) -->
                    <button onclick="openStaffLoginPanel()" class="btn-staff px-8 py-3 rounded-xl font-semibold text-white flex items-center justify-center gap-2 min-w-[180px]">
                        <i class="fas fa-users-cog"></i> Staff Login
                    </button>

                    <!-- REGISTER -->
                    <button onclick="openRegisterPanel()" class="btn-secondary px-8 py-3 rounded-xl font-semibold text-white flex items-center justify-center gap-2 min-w-[180px]">
...
// ============ AUTO-DASH STUDENT ID (habang nagta-type) ============
document.getElementById('login_username')?.addEventListener('input', function() {
    const errorEl = document.getElementById('studentIdError');
    const pattern = /^\d{4}-\d{5}$/;
    ...
    // Limit to 9 digits (4 + 5)
    if (digits.length > 9) {
        digits = digits.substring(0, 9);
    }
    
    // Build formatted value with auto-dash
    let formatted = '';
    if (digits.length > 0) {
    ...
    if (this.value.length > oldValue.length && cursorPos === 4) {
        // If user typed the 5th digit, move cursor past the dash
...
document.getElementById('login_username')?.addEventListener('keydown', function(e) {
   ...
   // ============ STAFF/ADMIN/SUPPORT LOGIN FORM ============
document.getElementById('staffLoginForm')?.addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            showToast('Login successful! Redirecting...', 'success');
            // ✅ I-FORCE ANG REDIRECT SA /staff/dashboard
    ...
    document.getElementById('resetForm')?.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                setTimeout(() => { closeAllPanels(); openLoginPanel(); }, 1500);
            } else {
    ...
    document.getElementById('newEmailVerifyBtn')?.addEventListener('click', async function() {
        ...
        try {
            ...
            if (data.success) {
                showToast(data.message, 'success');
                setTimeout(() => { closeNewEmailVerifyPanel(); openLoginPanel(); tempRegistrationData = null; }, 1500);
            } else {
                showToast(data.message, 'error');
                document.querySelectorAll('#newEmailVerifyPanel .new-otp-digit').forEach(d => d.value = '');
    ...
    issueTypeButton?.addEventListener('click', function(e) {
        e.stopPropagation();
    ...
    document.getElementById('panelOverlay')?.addEventListener('click', function(e) {
        if (e.target === this) closeAllPanels();
    });
    ...
    const translations = {
        en: {
            ...
            publicBeta: "PUBLIC BETA 2.0",
            mainTitle: "Clearance System",
