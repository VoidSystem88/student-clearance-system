...
<?php $__env->startSection('header', 'Support Requests'); ?>
    ...
    <div class="overflow-x-auto">
        <table class="w-full" id="requestsTable">
            ...
            <tbody class="divide-y divide-gray-100" id="requestsTableBody">
                <?php $__empty_1 = true; $__currentLoopData = $supportRequests ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $request): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-5 py-3 text-sm font-mono text-gray-600">#<?php echo e($request->id); ?></td>
                    <td class="px-5 py-3">
                        <div class="font-medium text-gray-800"><?php echo e($request->student->first_name ?? ''); ?> <?php echo e($request->student->last_name ?? ''); ?></div>
                        <div class="text-xs text-gray-500"><?php echo e($request->student->student_id ?? ''); ?></div>
                        <div class="text-xs text-gray-400"><?php echo e($request->student->email ?? ''); ?></div>
                    </td>
                    <td class="px-5 py-3">
                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium
                            <?php if($request->request_type == 'password_reset'): ?> bg-blue-100 text-blue-700
                            <?php elseif($request->request_type == 'account_id_reset'): ?> bg-purple-100 text-purple-700
                            <?php elseif($request->request_type == 'login_issue'): ?> bg-red-100 text-red-700
                            <?php elseif($request->request_type == 'otp_issue'): ?> bg-yellow-100 text-yellow-700
                            <?php elseif($request->request_type == 'clearance_issue'): ?> bg-orange-100 text-orange-700
                            <?php else: ?> bg-gray-100 text-gray-700 <?php endif; ?>">
                            <?php echo e(str_replace('_', ' ', ucfirst($request->request_type))); ?>

                        </span>
                    </td>
                    <td class="px-5 py-3 text-sm text-gray-600 max-w-xs">
                        <p class="line-clamp-2"><?php echo e(Str::limit($request->description, 80)); ?></p>
                    </td>
                    <td class="px-5 py-3">
                        <?php if($request->attachment_path): ?>
                            ...
                            <?php if($isImage): ?>
                                <button onclick="openImageViewer('<?php echo e($fileUrl); ?>', '<?php echo e(addslashes($request->student->first_name . ' ' . $request->student->last_name)); ?>', '<?php echo e($filename); ?>')" 
                                        class="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 text-sm transition">
                                    <i class="fas fa-image"></i> View Image
                                </button>
                                <a href="<?php echo e(url('/file/' . $filename . '?download=1')); ?>" 
                    ...
                    <td class="px-5 py-3">
                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium
                            ...
                            <?php elseif($request->status == 'resolved'): ?> bg-green-100 text-green-700
                            <?php else: ?> bg-gray-100 text-gray-700 <?php endif; ?>">
                            <i class="fas 
                                <?php if($request->status == 'pending'): ?> fa-clock
                                <?php elseif($request->status == 'in_progress'): ?> fa-spinner fa-pulse
                                <?php elseif($request->status == 'resolved'): ?> fa-check-circle
                    ...
                    <td class="px-5 py-3 text-sm text-gray-500">
                        ...
                        <br><span class="text-xs"><?php echo e($request->created_at->format('h:i A')); ?></span>
                    </td>
                    <td class="px-5 py-3">
                        <button onclick="openRequestModal(<?php echo e($request->id); ?>)" 
...
<?php echo $__env->make('layouts.support', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/vol15_6/infinityfree.com/if0_42013478/voidclearancesystem.gt.tc/htdocs/resources/views/support/requests.blade.php ENDPATH**/ ?>