...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="stat-card bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="flex justify-between items-start">
            <div><p class="text-gray-500 text-sm">Total Requests</p><p class="text-2xl font-bold text-blue-600 mt-1"><?php echo e(($pendingCount ?? 0) + ($approvedCount ?? 0) + ($rejectedCount ?? 0)); ?></p></div>
    ...
    <div class="stat-card bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="flex justify-between items-start">
            <div><p class="text-gray-500 text-sm">Pending</p><p class="text-2xl font-bold text-yellow-600 mt-1"><?php echo e($pendingCount ?? 0); ?></p></div>
    ...
    <div class="stat-card bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="flex justify-between items-start">
            <div><p class="text-gray-500 text-sm">Approved</p><p class="text-2xl font-bold text-green-600 mt-1"><?php echo e($approvedCount ?? 0); ?></p></div>
    ...
    <div class="stat-card bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="flex justify-between items-start">
            <div><p class="text-gray-500 text-sm">Rejected</p><p class="text-2xl font-bold text-red-600 mt-1"><?php echo e($rejectedCount ?? 0); ?></p></div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="border-b border-gray-200 px-4 pt-3 flex flex-wrap gap-1 overflow-x-auto">
        <button class="tab-btn px-4 py-3 text-sm font-medium border-b-2 transition whitespace-nowrap active" data-tab="pending-tab" style="color: #3b82f6; border-bottom-color: #3b82f6;">
            <i class="fas fa-clock text-yellow-500 mr-1"></i> Pending <span class="bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full text-xs ml-1"><?php echo e($pendingCount ?? 0); ?></span>
        </button>
        <button class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700 transition whitespace-nowrap" data-tab="approved-tab">
            <i class="fas fa-check-circle text-green-500 mr-1"></i> Approved <span class="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs ml-1"><?php echo e($approvedCount ?? 0); ?></span>
        </button>
        <button class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700 transition whitespace-nowrap" data-tab="reports-tab">
            <i class="fas fa-file-alt text-purple-500 mr-1"></i> Reports <span class="bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full text-xs ml-1"><?php echo e(isset($allReports) ? $allReports->count() : 0); ?></span>
        </button>
        <button class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700 transition whitespace-nowrap" data-tab="requirements-tab">
            <i class="fas fa-list-check text-blue-500 mr-1"></i> Requirements <span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-xs ml-1"><?php echo e(isset($requirements) ? $requirements->count() : 0); ?></span>
        </button>
        <button class="tab-btn px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700 transition whitespace-nowrap" data-tab="rejected-tab">
            <i class="fas fa-times-circle text-red-500 mr-1"></i> Rejected <span class="bg-red-100 text-red-700 px-2 py-0.5 rounded-full text-xs ml-1"><?php echo e($rejectedCount ?? 0); ?></span>
        </button>
    </div>
    ...
    <div class="p-4">
        ...
        <div id="pending-tab" class="tab-content">
            <?php if(($pendingRequests ?? collect([]))->count() > 0): ?>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="table-row transition">
                                <td class="px-4 py-3"><div class="font-medium text-gray-800"><?php echo e($req->student->first_name ?? 'Unknown'); ?> <?php echo e($req->student->last_name ?? ''); ?></div><div class="text-xs text-gray-500"><?php echo e($req->student->email ?? ''); ?></div></td>
                                <td class="px-4 py-3 font-mono text-gray-600"><?php echo e($req->student->student_id ?? 'N/A'); ?></td>
                                <td class="px-4 py-3"><span class="text-gray-600"><?php echo e($req->student->course ?? 'N/A'); ?></span><br><span class="text-xs text-gray-400"><?php echo e($req->student->year_level ?? 'N/A'); ?></span></td>
                                <td class="px-4 py-3 text-gray-500 text-xs"><?php echo e($req->submitted_at ? $req->submitted_at->format('M d, Y') : 'N/A'); ?></td>
                                <td class="px-4 py-3">
                                    <?php if($req->attachment_path): ?>
                                        ...
                                        ?>
                                        <button onclick="viewPhoto('<?php echo e($path); ?>')" class="text-blue-600 hover:text-blue-800 transition">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex gap-2">
                                        <form method="POST" action="<?php echo e(route('staff.approve', $req->id)); ?>" class="inline"><?php echo csrf_field(); ?><button type="submit" class="btn-approve bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium"><i class="fas fa-check mr-1"></i> Approve</button></form>
                                        <button onclick="showRejectModal(<?php echo e($req->id); ?>)" class="btn-reject bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium"><i class="fas fa-times mr-1"></i> Reject</button>
                                    </div>
                                </td>
                            </tr>
        ...
        <div id="approved-tab" class="tab-content hidden">
            <?php if(($approvedRequests ?? collect([]))->count() > 0): ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php $__currentLoopData = $approvedRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $req): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="table-row transition">
                                <td class="px-4 py-3"><div class="font-medium text-gray-800"><?php echo e($req->student->first_name ?? 'Unknown'); ?> <?php echo e($req->student->last_name ?? ''); ?></div><div class="text-xs text-gray-500"><?php echo e($req->student->email ?? ''); ?></div></td>
                                <td class="px-4 py-3 font-mono text-gray-600"><?php echo e($req->student->student_id ?? 'N/A'); ?></td>
                                <td class="px-4 py-3 text-gray-600"><?php echo e($req->student->course ?? 'N/A'); ?></td>
                                <td class="px-4 py-3 text-gray-600"><?php echo e($req->student->year_level ?? 'N/A'); ?></td>
                                <td class="px-4 py-3 text-gray-500 text-xs"><?php echo e($req->updated_at ? $req->updated_at->format('M d, Y h:i A') : 'N/A'); ?></td>
            ...
            <?php if(isset($allReports) && $allReports->count() > 0): ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="report-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div class="px-5 py-4 border-b border-gray-200 flex justify-between items-start <?php if($report->status == 'pending'): ?> bg-yellow-50 <?php elseif($report->status == 'approved'): ?> bg-green-50 <?php else: ?> bg-red-50 <?php endif; ?>">
                            <div class="flex-1 min-w-0"><h4 class="font-semibold text-gray-800 truncate"><?php echo e($report->report_title ?? 'Untitled Report'); ?></h4><p class="text-xs text-gray-500"><i class="far fa-calendar-alt mr-1"></i> <?php echo e(\Carbon\Carbon::parse($report->created_at)->format('M d, Y h:i A')); ?></p></div>
                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium <?php if($report->status == 'pending'): ?> bg-yellow-200 text-yellow-800 <?php elseif($report->status == 'approved'): ?> bg-green-200 text-green-800 <?php else: ?> bg-red-200 text-red-800 <?php endif; ?>"><i class="fas <?php if($report->status == 'pending'): ?> fa-clock <?php elseif($report->status == 'approved'): ?> fa-check-circle <?php else: ?> fa-times-circle <?php endif; ?> text-xs"></i> <?php echo e(ucfirst($report->status)); ?></span>
                        </div>
                        <div class="p-5">
                            <div class="grid grid-cols-2 gap-3 text-sm">
                                <div><p class="text-xs text-gray-500">Event</p><p class="font-medium text-gray-800"><?php echo e($report->event_name ?? 'N/A'); ?></p></div>
                                <div><p class="text-xs text-gray-500">Students</p><p class="font-medium text-gray-800"><?php echo e($report->total_students ?? 0); ?></p></div>
                                <div class="col-span-2"><p class="text-xs text-gray-500">Sent By</p><?php $officer = App\Models\User::find($report->officer_id); ?><p class="font-medium text-gray-800"><?php echo e($officer->name ?? 'Unknown Officer'); ?></p></div>
                                <?php if($report->notes): ?><div class="col-span-2"><p class="text-xs text-gray-500">Notes</p><p class="text-sm text-gray-600"><?php echo e(Str::limit($report->notes, 80)); ?></p></div><?php endif; ?>
                            </div>
                            <div class="mt-4 pt-3 border-t border-gray-100 flex flex-wrap gap-2">
                                <?php if($report->status == 'pending'): ?>
                                    <button onclick="approveReport(<?php echo e($report->id); ?>)" class="btn-approve bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium inline-flex items-center gap-1"><i class="fas fa-check"></i> Verify & Approve</button>
                                    <button onclick="rejectReport(<?php echo e($report->id); ?>)" class="btn-reject bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium inline-flex items-center gap-1"><i class="fas fa-times"></i> Reject</button>
                                <?php endif; ?>
                                <?php if($report->status == 'approved'): ?>
                                    <button onclick="exportReport(<?php echo e($report->id); ?>)" class="btn-export bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium inline-flex items-center gap-1"><i class="fas fa-file-csv"></i> Extract CSV</button>
                                <?php endif; ?>
                                <button onclick="deleteReport(<?php echo e($report->id); ?>)" class="btn-delete bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium inline-flex items-center gap-1"><i class="fas fa-trash-alt"></i> Delete</button>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div id="requirements-tab" class="tab-content hidden">
            ...
            <?php if(isset($requirements) && $requirements->count() > 0): ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="table-row transition">
                                <td class="px-4 py-3"><span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium"><?php echo e($req->year_level); ?></span></td>
                                <td class="px-4 py-3 font-medium text-gray-800"><?php echo e($req->requirement_name); ?></td>
                                <td class="px-4 py-3"><?php if($req->is_required): ?><span class="text-green-600"><i class="fas fa-check-circle"></i> Required</span><?php else: ?><span class="text-gray-400">Optional</span><?php endif; ?></td>
                                <td class="px-4 py-3">
                                    <div class="flex gap-2">
                                        <button onclick="editRequirement(<?php echo e($req->id); ?>, '<?php echo e(addslashes($req->requirement_name)); ?>', '<?php echo e($req->year_level); ?>', <?php echo e($req->is_required); ?>)" class="btn-edit bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium"><i class="fas fa-edit"></i></button>
                                        <button onclick="deleteRequirement(<?php echo e($req->id); ?>)" class="btn-delete bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium"><i class="fas fa-trash-alt"></i></button>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div id="rejected-tab" class="tab-content hidden">
            <?php if(($rejectedRequests ?? collect([]))->count() > 0): ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php $__currentLoopData = $rejectedRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $req): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="table-row transition">
                                <td class="px-4 py-3"><div class="font-medium text-gray-800"><?php echo e($req->student->first_name ?? 'Unknown'); ?> <?php echo e($req->student->last_name ?? ''); ?></div></td>
                                <td class="px-4 py-3 font-mono text-gray-600"><?php echo e($req->student->student_id ?? 'N/A'); ?></td>
                                <td class="px-4 py-3 text-gray-600"><?php echo e($req->student->course ?? 'N/A'); ?></td>
                                <td class="px-4 py-3 text-gray-500 text-xs"><?php echo e($req->updated_at ? $req->updated_at->format('M d, Y') : 'N/A'); ?></td>
                                <td class="px-4 py-3 text-xs text-red-600"><?php echo e($req->remarks ?? 'No remarks'); ?></td>
                            </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </tbody>
                    </table>
...
<div id="requirementModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <form id="requirementForm"><?php echo csrf_field(); ?><input type="hidden" id="reqId">
...
<script>
    ...
    function openAddRequirementModal() {
        ...
        document.getElementById('requirementForm').action = '<?php echo e(route("staff.requirements.store")); ?>';
    ...
    document.getElementById('requirementForm')?.addEventListener('submit', function(e) {
        ...
        const url = id ? `/staff/requirements/${id}` : '<?php echo e(route("staff.requirements.store")); ?>';
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.staff', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/vol15_6/infinityfree.com/if0_42013478/voidclearancesystem.gt.tc/htdocs/resources/views/staff/dashboard.blade.php ENDPATH**/ ?>