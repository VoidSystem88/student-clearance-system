...
@section('header', 'Request Assistance')
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden sticky top-20">
                ...
                <div id="requestsList" class="divide-y divide-gray-100 dark:divide-gray-700 max-h-[600px] overflow-y-auto">
                    ...
                    <div class="assistance-item p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition" data-id="{{ $req->id }}">
                        <div class="flex justify-between items-start mb-1">
                            <span class="text-sm font-medium text-gray-800 dark:text-white flex items-center gap-1">
                                ...
                                @elseif($req->request_type == 'account_id_reset')
                                    <i class="fas fa-id-card text-purple-500"></i>
                                @elseif($req->request_type == 'login_issue')
                                    <i class="fas fa-sign-in-alt text-red-500"></i>
                                @elseif($req->request_type == 'otp_issue')
                                    <i class="fas fa-envelope text-yellow-500"></i>
                                @elseif($req->request_type == 'clearance_issue')
                            ...
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium
                                @if($req->status == 'pending') bg-yellow-100 dark:bg-yellow-300/30 text-yellow-700 dark:text-yellow-500
                                @elseif($req->status == 'in_progress') bg-blue-100 dark:bg-blue-300/30 text-blue-700 dark:text-blue-500
                                @elseif($req->status == 'resolved') bg-green-100 dark:bg-green-300/30 text-green-700 dark:text-green-500
                                @else bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 @endif">
                                <i class="fas 
                                    @if($req->status == 'pending') fa-clock
                                    @elseif($req->status == 'in_progress') fa-spinner fa-pulse
                                    @elseif($req->status == 'resolved') fa-check-circle
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            ...
                            @if($req->is_edited ?? false)
                                <span class="text-blue-500 ml-2"><i class="fas fa-edit"></i> Edited</span>
                            @endif
                        </p>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-2 line-clamp-2 assistance-description">{{ Str::limit($req->description, 80) }}</p>
                        
                        @if($req->attachment_path)
                            <a href="{{ route('student.assistance.attachment', $req->id) }}" target="_blank" class="text-xs text-blue-600 dark:text-blue-400 hover:underline mt-2 inline-block">
                        ...
                        @if($req->response_message)
                            <p class="text-xs text-blue-600 dark:text-blue-400 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                                <i class="fas fa-reply mr-1"></i> {{ Str::limit($req->response_message, 60) }}
                            </p>
                        @endif
                        ...
                        <div class="assistance-actions">
                            <button onclick="editRequest({{ $req->id }})" 
                        ...
                        <div class="mt-2 text-xs text-gray-400 dark:text-gray-500">
                            <i class="fas fa-lock mr-1"></i> Cannot edit/delete - Request is {{ $req->status }}
                        </div>
                        @endif
                    </div>
                    @empty
