    ...
    <div class="rounded-xl p-3 border" style="background: rgba(34,197,94,0.1); border-color: rgba(34,197,94,0.2);">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xl font-bold" style="color: #15803d;">{{ $approvedCount ?? 0 }}</p>
            </div>
    ...
    <div class="rounded-xl p-3 border" style="background: rgba(234,179,8,0.1); border-color: rgba(234,179,8,0.2);">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xl font-bold" style="color: #a16207;">{{ $pendingCount ?? 0 }}</p>
            </div>
    ...
    <div class="rounded-xl p-3 border" style="background: rgba(239,68,68,0.1); border-color: rgba(239,68,68,0.2);">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xl font-bold" style="color: #b91c1c;">{{ $rejectedCount ?? 0 }}</p>
            </div>
    ...
    <div class="rounded-xl p-3 border" style="background: rgba(107,114,128,0.1); border-color: rgba(107,114,128,0.2);">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xl font-bold" style="color: #4b5563;">{{ $notSubmittedCount ?? 0 }}</p>
            </div>
...
<div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-6">
    <div class="flex justify-between items-center mb-2">
        ...
        <span class="text-xs text-gray-500">{{ $approvedCount ?? 0 }}/{{ $totalDepartments ?? 0 }} cleared</span>
    </div>
    <div class="w-full bg-gray-200 rounded-full h-2">
        <div class="bg-green-500 h-2 rounded-full transition-all" style="width: {{ isset($totalDepartments) && $totalDepartments > 0 ? ($approvedCount ?? 0) / $totalDepartments * 100 : 0 }}%"></div>
    </div>
    @if(isset($isFullyCleared) && $isFullyCleared)
    <div class="mt-4 grid grid-cols-2 gap-3">
        <a href="{{ route('student.clearance.view-pdf') }}" target="_blank" class="flex items-center justify-center gap-1 bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 transition text-xs font-medium"><i class="fas fa-eye text-xs"></i> View Slip</a>
        <a href="{{ route('student.clearance.print') }}" class="flex items-center justify-center gap-1 bg-green-600 text-white px-3 py-2 rounded-lg hover:bg-green-700 transition text-xs font-medium"><i class="fas fa-download text-xs"></i> Download PDF</a>
    </div>
    @endif
</div>
...
<div class="space-y-3">
    @forelse($departments ?? [] as $department)
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-100" style="background: {{ $c['bg'] }};">
                <div class="flex justify-between items-start">
                    <div class="flex-1">
                        <h4 class="font-semibold text-gray-800 text-sm">{{ $department->name }}</h4>
                        @if($department->handler_name)<div class="text-xs text-gray-500 mt-0.5">{{ $department->handler_name }}</div>@endif
                    </div>
                    <div class="ml-2">
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium" style="background: {{ $c['badgeBg'] }}; color: {{ $c['badgeText'] }};">
                            @if($status == 'approved')<i class="fas fa-check-circle text-xs"></i> Approved
                            @elseif($status == 'rejected')<i class="fas fa-times-circle text-xs"></i> Rejected
                            @elseif($status == 'pending')<i class="fas fa-clock text-xs"></i> Pending
                            @else<i class="fas fa-hourglass-start text-xs"></i> Not Submitted
                            @endif
                        </span>
                    </div>
                </div>
            ...
            <div class="px-4 py-3 space-y-3">
                <div class="flex justify-between items-center text-xs"><span class="text-gray-500">Submitted Date:</span><span class="text-gray-700">{{ $request ? $request->submitted_at->format('M d, Y') : '—' }}</span></div>
                
                @if($status == 'rejected' && $request->remarks)
                <div class="rounded-lg p-2" style="background: rgba(239,68,68,0.1);">
                    <p class="text-xs" style="color: #dc2626;"><i class="fas fa-comment-dots mr-1"></i><strong>Reason:</strong> {{ $request->remarks }}</p>
                </div>
                @endif
                
                <div class="flex gap-2 pt-1">
                    <button onclick="viewRequirements({{ $department->id }}, '{{ addslashes($department->name) }}')" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-gray-600 text-white text-xs rounded-lg hover:bg-gray-700 transition"><i class="fas fa-list-check text-xs"></i> Requirements</button>
                    
                    @if(!$request || $status == 'rejected')
                        <button onclick="showSubmitForm({{ $department->id }}, '{{ addslashes($department->name) }}')" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-blue-600 text-white text-xs rounded-lg hover:bg-blue-700 transition"><i class="fas fa-paper-plane text-xs"></i> Submit</button>
                    @elseif($status == 'pending')
                        <button onclick="cancelRequest({{ $request->id }})" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-red-600 text-white text-xs rounded-lg hover:bg-red-700 transition"><i class="fas fa-times-circle text-xs"></i> Cancel</button>
