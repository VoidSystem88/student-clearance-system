...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="mx-5 my-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
        ...
        <form method="POST" action="{{ route('support.bulk.year.update') }}" id="bulkUpdateForm">
            @csrf
                ...
                <div class="border rounded-lg max-h-64 overflow-y-auto bg-gray-50">
                    <table class="w-full text-sm">
                        ...
                        <tbody id="selectedStudentsTableBody">
                            ...
                            <tr class="student-row border-b hover:bg-gray-100" data-name="{{ strtolower($student->first_name . ' ' . $student->last_name) }}" data-id="{{ $student->id }}" data-student-id="{{ $student->student_id }}">
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2 font-mono text-xs">{{ $student->student_id }}</td>
                                <td class="px-3 py-2">{{ $student->course }}</td>
                                <td class="px-3 py-2">
                                    <span class="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded-full text-xs">{{ $student->year_level }}</span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                ...
                <select name="course_year" id="edit_course_year" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
                    ...
                    @foreach($courses ?? [] as $course)
                        <optgroup label="{{ $course->code }} - {{ $course->name }}">
                            <option value="{{ $course->code }} - 1st Year">{{ $course->code }} - 1st Year</option>
                            <option value="{{ $course->code }} - 2nd Year">{{ $course->code }} - 2nd Year</option>
                            <option value="{{ $course->code }} - 3rd Year">{{ $course->code }} - 3rd Year</option>
                            <option value="{{ $course->code }} - 4th Year">{{ $course->code }} - 4th Year</option>
                        </optgroup>
                    @endforeach
                </select>
            </div>
