    ...
    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-red-600">{{ $pendingBugReports ?? 0 }}</p>
            </div>
        ...
        <table class="w-full" id="bugReportsTable">
            ...
            <tbody class="divide-y divide-gray-100" id="bugReportsTableBody">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-5 py-3 text-sm font-mono text-gray-600">#{{ $report->id }}</td>
                    <td class="px-5 py-3">
                        <div class="font-medium text-gray-800">{{ $report->name ?? 'Anonymous' }}</div>
                        <div class="text-xs text-gray-500">{{ $report->email }}</div>
                        <div class="text-xs text-gray-400">{{ $report->student_id ?? 'No ID' }}</div>
                    </td>
                    <td class="px-5 py-3">
                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium
                            @if($report->type == 'bug') bg-red-100 text-red-700
                            @elseif($report->type == 'login_issue') bg-yellow-100 text-yellow-700
                            @elseif($report->type == 'registration_issue') bg-orange-100 text-orange-700
                            @elseif($report->type == 'clearance_issue') bg-purple-100 text-purple-700
                            @else bg-gray-100 text-gray-700 @endif">
                            {{ ucfirst(str_replace('_', ' ', $report->type)) }}
                        </span>
                    </td>
                    <td class="px-5 py-3 text-sm text-gray-600 max-w-xs">{{ Str::limit($report->message, 60) }}</td>
                    <td class="px-5 py-3">
                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium
                            ...
                            <i class="fas 
                                ...
                                @elseif($report->status == 'reviewed') fa-eye
                                @else fa-check @endif text-xs"></i>
                            {{ ucfirst($report->status) }}
                        </span>
                    </td>
                    <td class="px-5 py-3 text-sm text-gray-500">{{ $report->created_at->format('M d, Y h:i A') }}</td>
                    <td class="px-5 py-3">
                        <button onclick="viewBugReport({{ $report->id }}, '{{ addslashes($report->message) }}', '{{ addslashes($report->name ?? 'Anonymous') }}', '{{ $report->email }}', '{{ $report->student_id ?? 'N/A' }}', '{{ $report->type }}', '{{ $report->status }}', '{{ addslashes($report->admin_response ?? '') }}')" 
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6" id="cardsContainer">
    <div class="bg-blue-50 rounded-xl p-6 text-center card-item" data-name="requests support tickets manage view">
        ...
        <a href="{{ route('support.requests') }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
    ...
    <div class="bg-green-50 rounded-xl p-6 text-center card-item" data-name="students manage reset password accounts">
        ...
        <a href="{{ route('support.students') }}" class="inline-flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">
    ...
    <div class="bg-purple-50 rounded-xl p-6 text-center card-item" data-name="feedbacks respond suggestions comments">
        ...
        <a href="{{ route('support.feedbacks') }}" class="inline-flex items-center gap-2 bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700">
