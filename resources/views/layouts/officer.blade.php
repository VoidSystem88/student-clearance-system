...
<div class="app-wrapper">
    ...
    <header class="top-header">
        <div class="flex items-center gap-3">
            ...
            <span class="page-title">@yield('header', 'Officer Dashboard')</span>
        </div>
    ...
    <nav class="bottom-nav" id="bottomNav">
        <a href="{{ route('officer.dashboard') }}" 
           class="nav-item {{ request()->routeIs('officer.dashboard') ? 'active' : '' }}">
            <i class="fas fa-home"></i>
            <span>Home</span>
        </a>

        <a href="{{ route('officer.students') }}" 
           class="nav-item {{ request()->routeIs('officer.students') ? 'active' : '' }}">
        ...
        <a href="{{ route('officer.verified') }}" 
           class="nav-item {{ request()->routeIs('officer.verified') ? 'active' : '' }}">
            <i class="fas fa-check-circle"></i>
            <span>Verified</span>
            @if(isset($verifiedCount) && $verifiedCount > 0)
                <span class="badge" style="background: #22c55e;">{{ $verifiedCount }}</span>
            @endif
        </a>

        <a href="{{ route('officer.send.report') }}" 
           class="nav-item {{ request()->routeIs('officer.send.report') ? 'active' : '' }}">
...
<script>
    ...
    function fetchNotifications() {
        fetch('{{ route("officer.notifications") }}')
    ...
    function markAsRead(id) {
        fetch('{{ route("officer.notification.mark-read") }}', {
            method: 'POST',
            headers: {
    ...
    function markAllAsRead() {
        fetch('{{ route("officer.notification.mark-all-read") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
