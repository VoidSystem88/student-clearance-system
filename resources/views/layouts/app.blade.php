...
<div class="app-container">
    <aside id="sidebar" class="sidebar">
        ...
        <div class="sidebar-nav">
            @auth
                <a href="{{ route('student.dashboard') }}" class="nav-link"><i class="fas fa-tachometer-alt w-5"></i><span>Dashboard</span></a>
                <a href="{{ route('student.clearance') }}" class="nav-link"><i class="fas fa-clipboard-list w-5"></i><span>Clearance</span></a>
                <a href="{{ route('student.reminders') }}" class="nav-link"><i class="fas fa-bell w-5"></i><span>Reminders</span></a>
                <a href="{{ route('student.profile') }}" class="nav-link"><i class="fas fa-user-circle w-5"></i><span>My Profile</span></a>
                <a href="{{ route('student.feedback') }}" class="nav-link"><i class="fas fa-star w-5"></i><span>Feedback</span></a>
                <div class="border-t my-3 mx-5 border-gray-200 dark:border-gray-700"></div>
                <form method="POST" action="{{ route('logout') }}">@csrf<button type="submit" class="nav-link text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 w-full text-left"><i class="fas fa-sign-out-alt w-5"></i><span>Logout</span></button></form>
            @endauth
        </div>
        <div class="sidebar-footer">
            ...
            <a href="{{ route('student.assistance') }}" class="nav-link assistance-link"><i class="fas fa-headset w-5"></i><span>Request Assistance</span></a>
            <div class="copyright"><p>© {{ date('Y') }} Clearance System</p><p>v1.0 | Void</p></div>
        </div>
    </aside>
    ...
    <div class="flex items-center gap-1">
        ...
        <h2 class="text-lg font-semibold text-gray-800 dark:text-white truncate">@yield('header', 'Dashboard')</h2>
    </div>
    ...
    <div class="flex items-center gap-2 sm:gap-4 flex-shrink-0">
        ...
        <span class="text-sm hidden sm:block text-gray-600 dark:text-gray-400">@auth {{ Auth::user()->first_name ?? '' }} @endauth</span>
        <div style="position: relative; display: inline-block; flex-shrink: 0;">
            <img id="profilePicture" class="profile-img cursor-pointer" src="{{ Auth::user()->profile_photo_url ?? 'https://ui-avatars.com/api/?background=3b82f6&color=fff&name=' . urlencode(Auth::user()->first_name ?? 'User') . '&size=64' }}" alt="Profile" data-user-id="{{ Auth::id() }}">
        ...
        <div class="p-4 md:p-6">@yield('content')</div>
    </div>
</div>

<div id="notificationDropdown"><div class="notification-header"><h4><i class="fas fa-bell mr-2"></i> Notifications</h4></div><div id="notificationList" class="notification-list"><div class="empty-notification"><i class="fas fa-bell-slash"></i><p>Loading notifications...</p></div></div><div class="notification-footer"><a href="{{ route('student.reminders') }}">View all announcements →</a></div></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileImg) { profileImg.addEventListener('click', () => { window.location.href = '{{ route("student.profile") }}'; }); }
...
@stack('scripts')
    @auth
    @if(session('show_ai_popup'))
        @php session()->forget('show_ai_popup'); @endphp
