...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="p-6">
            ...
            <div class="bg-blue-50 p-4 rounded-lg mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-blue-600">{{ $verifiedCount ?? 0 }}</p>
                    </div>
            ...
            <form method="POST" action="{{ route('officer.send.report') }}" enctype="multipart/form-data" id="sendReportForm">
                @csrf
                
                {{-- Department (Auto - Cannot be changed) --}}
                <div class="mb-4">
                ...
                <div class="bg-yellow-50 p-4 rounded-lg mb-6">
                    <p class="text-sm text-yellow-700 flex items-start gap-2">
                        ...
                        <span>The report will be sent directly to <strong>{{ $department->name ?? 'your department' }}</strong>. 
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="table-row transition">
                        <td class="px-4 py-3 text-gray-500">{{ \Carbon\Carbon::parse($report->created_at)->format('M d, Y h:i A') }}</td>
                        <td class="px-4 py-3 text-gray-800">{{ $report->report_title ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $report->department_name ?? $department->name ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $report->total_students ?? 0 }}</td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium
                                @if($report->status == 'sent') badge-sent
                                @elseif($report->status == 'pending') badge-pending
                                @else badge-failed @endif">
                                <i class="fas 
                                    @if($report->status == 'sent') fa-check-circle
                                    @elseif($report->status == 'pending') fa-clock
