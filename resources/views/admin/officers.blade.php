...
@section('header', 'Manage Officers')
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    <td class="p-3">{{ $officer->id }}</td>
                    <td class="p-3 font-semibold">{{ $officer->name }}</td>
                    <td class="p-3">{{ $officer->email }}</td>
                    <td class="p-3">{{ $officer->department->name ?? 'N/A' }}</td>
                    <td class="p-3">
                    ...
                    <td class="p-3">
                        <button onclick="openEditModal({{ $officer->id }})" class="bg-yellow-500 text-white px-3 py-1 rounded text-sm hover:bg-yellow-600 transition">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button onclick="confirmDelete({{ $officer->id }}, '{{ addslashes($officer->name) }}')" 
                                class="bg-red-600 text-white px-3 py-1 rounded text-sm hover:bg-red-700 transition">
                            <i class="fas fa-trash"></i> Delete
                        </button>
                        <button onclick="toggleStatus({{ $officer->id }}, '{{ addslashes($officer->name) }}', {{ $officer->is_active ? 'true' : 'false' }})" 
                                class="bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700 transition">
                            <i class="fas {{ $officer->is_active ? 'fa-ban' : 'fa-check' }}"></i>
                            {{ $officer->is_active ? 'Deactivate' : 'Activate' }}
                        </button>
                        <form id="delete-form-{{ $officer->id }}" method="POST" action="{{ route('admin.officer.destroy', $officer->id) }}" class="hidden">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
...
<script>
    ...
    function openAddModal() {
        ...
        document.getElementById('officerForm').action = '{{ route("admin.officer.store") }}';
