        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="p-3 font-semibold">{{ $course->name }}</td>
                    <td class="p-3 text-gray-600">{{ $course->department ?? '—' }}</td>
                    <td class="p-3 text-gray-600">{{ $course->duration ?? '—' }}</td>
                    <td class="p-3">
                    ...
                    <td class="p-3">
                        <div class="flex gap-1">
                            ...
                            <button onclick="toggleCourseStatus({{ $course->id }}, '{{ addslashes($course->code) }}', {{ $course->is_active ? 'true' : 'false' }})" 
                                    class="inline-flex items-center gap-1 {{ $course->is_active ? 'bg-gray-500 hover:bg-gray-600' : 'bg-green-500 hover:bg-green-600' }} text-white px-3 py-1 rounded text-sm transition">
                                <i class="fas {{ $course->is_active ? 'fa-ban' : 'fa-check' }}"></i>
                                {{ $course->is_active ? 'Deactivate' : 'Activate' }}
                            </button>
                            <button onclick="confirmDeleteCourse({{ $course->id }}, '{{ addslashes($course->code) }}', {{ $course->student_count }})" 
                                    class="inline-flex items-center gap-1 bg-red-600 text-white px-3 py-1 rounded text-sm hover:bg-red-700 transition">
                                <i class="fas fa-trash"></i> Delete
                            </button>
                        </div>
                        <form id="delete-course-form-{{ $course->id }}" method="POST" action="{{ route('admin.course.destroy', $course->id) }}" class="hidden">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
...
<script>
    function openAddModal() {
        ...
        document.getElementById('courseForm').action = '{{ route("admin.course.store") }}';
